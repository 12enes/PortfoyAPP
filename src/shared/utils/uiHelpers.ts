...
export const getCurrencySymbol = (type: string, symbolOrName: string) => {
  if (symbolOrName) {
...
export const isUsdType = (type: string, name?: string) => {
  const sym = getCurrencySymbol(type, name || '');
  return sym === '$';
};

export const getConvertedValue = (val: number, assetOrType: any, currency: string, usdToTryRate: number, isUsdTypeFn: any) => {
  const type = typeof assetOrType === 'string' ? assetOrType : assetOrType.type;
  const name = typeof assetOrType === 'string' ? '' : (assetOrType.name || assetOrType.symbol);
  const isUsd = isUsdTypeFn(type, name);
  
  if (currency === '₺') {
    return isUsd ? val * usdToTryRate : val;
  } else {
    return isUsd ? val : val / usdToTryRate;
  }
};
